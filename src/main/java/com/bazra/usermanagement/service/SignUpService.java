...
//	}
	...
	public ResponseEntity<?> signUpMaster2(MasterSignupRequest2 request, Authentication authentication)
		...
		Optional<MasterAgentInfo> masterAgentInfoopOptional = masterAgentRepository.findByuserName(authentication.getName());
...
//		}
		...
		if (!orgTypeOptional.isPresent()) {
			return ResponseEntity.badRequest().body(new ResponseError("Not a valid organization type!"));
		}
		...
		Optional<AddressElement> optionaladdresselementSPECLOCATION = addressElementRepository.findBytitle("SPECIFIC LOCATION");
		...
		System.out.println("REGION    "   + addressElementREGION);
		AddressElement addressElementSUBCITY = optionaladdresselementREGION.get();
		System.out.println("SUBCITY    "   + addressElementSUBCITY);
		AddressElement addressElementWOREDA = optionaladdresselementREGION.get();
		System.out.println("WOREDA    "   + addressElementWOREDA);
		AddressElement addressElementHOUSENUMBER = optionaladdresselementREGION.get();
		System.out.println("HOUSENUMBER    "   + addressElementHOUSENUMBER);
		AddressElement addressElementSPECIFIC = optionaladdresselementREGION.get();
		System.out.println("SPECIFIC    "   + addressElementSPECIFIC);
		...
		Optional<AgentInfo> agentOptional = agentRepository.findByUsername(request.getAgentPhone());
		...
		if (request.getCity() != null) {
			...
			if (optionalOrganizationAddresses.size() >= 1) {
				...
				if (organizationAddress.getAddress()==null) {
					...
					System.out.println("orgcity"  + addressElement);
...
//			AddressElement addressElement = optionaladdresselementREGION.get();
			...
			if (optionalOrganizationAddresses.size() >= 1) {
				...
				if (organizationAddress.getAddress()==null) {
					...
					System.out.println("orgRegion"  + addressElementREGION);
					organizationAddress1.setIsactive(true);
					organizationAddress1.setRegisteredDate(LocalDate.now());
					organizationAddress1.setOrganizationInfo(organizationInfo2);
					organizationAddressRepository.save(organizationAddress1);
...
//System.out.println(optionalUserAddress.isEmpty());
		...
		if (request.getSubcity() != null) {
			...
			System.out.println("ssssUUBUBBB CCICIITTYYY");
			List<OrganizationAddress> optionalOrganizationAddresses = organizationAddressRepository
					.findByorganizationInfo(organizationInfo2);
			OrganizationAddress organizationAddress = new OrganizationAddress();
			System.out.println("before    "+organizationAddress);
			System.out.println("list size subcity    "+optionalOrganizationAddresses.size());
			if (optionalOrganizationAddresses.size() >= 1) {
				for (int i = 0; i < optionalOrganizationAddresses.size(); i++) {
					if (optionalOrganizationAddresses.get(i).getAddressElement().equals(addressElement)) {
						organizationAddress = optionalOrganizationAddresses.get(i);
				...
				System.out.println("middle    "+organizationAddress);
				if (organizationAddress.getAddress()==null) {
					OrganizationAddress organizationAddress1 = new OrganizationAddress();
					organizationAddress1.setAddress(request.getSubcity());
					organizationAddress1.setAddressElement(addressElement);
					organizationAddress1.setIsactive(true);
					System.out.println("orgSubCityyyyyyy"  + addressElement);
					organizationAddress1.setRegisteredDate(LocalDate.now());
					organizationAddress1.setOrganizationInfo(organizationInfo2);
					organizationAddressRepository.save(organizationAddress1);
				} else {
...
//				OrganizationAddress organizationAddress1 = new OrganizationAddress();
				...
				System.out.println("oooooooorgSubCity"  + addressElement);
		...
		if (request.getWoreda() != null) {
			...
			if (optionalOrganizationAddresses.size() >= 1) {
				...
				if (organizationAddress.getAddress()==null) {
					...
					organizationAddress1.setAddressElement(addressElement);
					System.out.println("orgWoreda"  + addressElement);
					organizationAddress1.setIsactive(true);
					organizationAddress1.setRegisteredDate(LocalDate.now());
					organizationAddress1.setOrganizationInfo(organizationInfo2);
					organizationAddressRepository.save(organizationAddress1);
...
//			UserInfoService.saveTREAD(treaduploadDir, treadName, tread);
		...
		Optional<Role> roleOptional = roleRepository.findByrolename("AGENT");
		if (!roleOptional.isPresent()) {
			return ResponseEntity.badRequest().body(new ResponseError("Role AGENT doesnot exist!"));
		}
		Optional<OrganizationInfo> orgOptional = organizationInfoRepository.findByorgName(request.getOrganizationName());
		...
		if (request.getPhoneNumber() != null) {
			...
			List<OrganizationContact> optionalOrganizationContacts = organizationContactRepository.findByorganizationInfo(organizationInfo2);
		...
		Optional<AccountType> accountTypeOptional = accountTypeRepository.findByaccounttype("MASTER AGENT");
...
System.out.println("Agent username" + agentusername);
		...
		return ResponseEntity
				.ok(new AgentSignUpResponse(organizationInfo2.getOrgName(),organizationInfo2.getOrganizationType().getOrgType(),agentInfo2.getName(),agentInfo2.getUsername(),pin));
		// return ResponseEntity.badRequest().body(new SignUpResponse("Enter your
		// business licence number "));
	}
